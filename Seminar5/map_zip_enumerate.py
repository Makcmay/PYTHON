...
# positive_numbers = list(filter(func, numbers)) #функция(тру ор фолс) плюс итерируемый список

# print(positive_numbers)

# my_range =list(range(10)) # создание листа (ЛХ лайф хак)
# print(my_range)
...
# int_nums = list(filter(str.isalpha, nums)) #isalpha вернет только буквы, пробел не пройдет
# print(int_nums)
...
# int_products = list(filter(lambda x: x[0] == 'i', products))
# print(int_products)
...
# products = ['IPad', 'Samsung Galaxy', 'iPhone', 'iRiver']
# def accepted(el):
#     return el.lower().startswith('i')   #el.lower() все с маленькой делаем, startswith('i') начиная с (указать с чего)

# products_sample = filter(accepted, products)
# print(type(products_sample)) # <class 'filter'>
# print(*products_sample) # iPad iPhone iRiver   *products_sample не обязательно заворачивать в лист, можно распаковать при помощи "*"
...
# large_numbers = filter(lambda x: x > 50, numbers)
# even_numbers = list(filter(lambda x: x % 2 == 0, numbers))

# print(positive_numbers)
# print(*large_numbers) # пасаковка без [скобок]
# print(even_numbers)
...
# new_words1 = [p for p in products if len(p) > 4] #[p for p in products if len(p) > 4] p for  добавить элемент из p in products если if len(p) > 4
# new_words4 = filter(lambda p: len(p) > 4, products)
# new_words2 = list(filter(lambda p: 'e' in p, products)) # если есть "е" в элементе, тогда тру
#
#
# new_words1 = [p for p in products if len(p) > 6] # [что откуда условие]
# new_dict = {i: p for i, p in enumerate(products) if len(p) > 4} # дикт компликейшен, словарь, i: ключ, р-значение, из p for i значение для ключ, в p in enumerate(products) значение в пронумерованном списке, при условии
...
# b = 5
# a, b = b, a
# print(a, b) множественное присваивание и смена занчения без доп переменной
'''
ЗАДАЧА
...
# products = ['iPad', 'Samsung Galaxy', 'iPhone', 'IRiver']
# print(*(filter(lambda x: x.lower().startswith('i'), products))) # отличный вариант
...
# new_numbers = list(map(func, numbers1, numbers2, numbers3))  # можно маптъить много элементов
...
# prices_tup = tuple(map(float, prices)) # tuple кортеж
...
# new_numbers1 = list(map(lambda x: x+1, numbers)) # мапим через лямбду, нет нужно пистаь отдельную функцию
...
# new_strings = list(map(lambda x, y: x*y, strings, numbers)) #  перемножаем строку на число ['aaa', 'bb', 'c', 'dddd', 'eeeee']
...
# print(list(map(lambda x: round((float(x) - (float(x)* d)/100), 2), prices))) # map(lambda x: round(float(x) * (100 - DISCOUNT) / 100, 2),prices)
...
# result = zip(numbers, words, russian) заварачивает в кортеж
# print(list(result))
...
# info = dict(zip(keys, values)) # можно сразу сделать словарь
# print(info)
...
# new_list = list(zip_longest(user_names, user_logins, user_roles)) # zip_longest поставит none в пустые поля и выведе всю длинну
# print(new_list)
...
#     print(f' {name} , {login} , {role} ') можно записать в файл
...
# for pair in enumerate(colors, 10):  # for pair in enumerate(colors, 10): можно указать с какого значения начинается нумерация
#     print(pair)
