...
def bot_calc(value): # бот определяет количество конфет
    candy = randint(1,28)
    
    while value-candy <= 28 and value > 29:
        candy = randint(1,28)
        print(candy,value-candy)
    return candy
...
while value > 28:           # пока общее число конфет больше 28 продолжаются ходы
    if player_turn:
        ...
        candy = bot_calc(value) # бот берет рандомное число от 1 до 28
        counter2 += candy
        value -= candy
        player_turn = True
